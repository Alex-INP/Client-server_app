...
class ServerMetaclass(type):
	def __init__(self, clsname, bases, clsdict):
		...
		if "connect" in methods:
			raise TypeError("You cannot use 'connect' method in server class instance")
...
class DocstringMetaclass(type):
	def __init__(self, clsname, bases, clsdict):
		if "__doc__" not in clsdict.keys():
			raise TypeError(f"Class '{clsname}' must have a documentation string.")

		no_dockstring_funcs = []
		for key, val in clsdict.items():
			if key[:2] != "__" and hasattr(val, "__call__") and not getattr(val, "__doc__"):
				no_dockstring_funcs.append(key)

		if no_dockstring_funcs:
			raise TypeError(f"Functions in class '{clsname}' must have a documentation string. Functions without documentation string: {no_dockstring_funcs}")
